...
3. **评测集泄题。** 96 例测试集里 **96 例的题面都含答案触发词**，一张 60 行关键词表就能考 100 分。在这套集子上的满分证明不了架构价值。详见 §10。
...
2. **禁止内部知识补全**：图里没有就输出 `missing_from_graph` / `no_candidate` / `unsupported`。哪怕"知道"DESeq2、Seurat 存在——不在 51 个闭集里就不许出现。
...
**作用**：接地校验——工具是否在 51 个闭集目录、文件名是否在图里、路径是否为图谱记录、队列号是否真实。
**参数**：`plan`（必需，对象或字符串）。
...
接手时有一套 96 例测试集，重版在上面拿 100%。

第一步不是优化，是检验这套题。用 78 词关键词表逐条比对题面与答案：

> **96 / 96（100%）的问题文本直接包含答案触发词。**

题目问"请做 WGCNA 共表达网络分析"，答案就是 `wgcna`。在这套集子上的满分不能区分任何系统。
...
| 臂 | 定义 | 作用 |
|---|---|---|
| floor | 永远猜最高频工具 | 随机线，性能下界 |
| **ceiling** | 60 行关键词表，无 AI | **纯规则的上限** |
| SUT | skill + read_cypher + 真实模型 | 被测系统 |
...
**去名集**：另建 70 题（14 工具 × 5），只写意图、不含任何规则触发词，并程序化验证与关键词表零重叠。例如不说"做 WGCNA"，改说"找出一组表达模式相似、并且和临床分期相关的基因"。

**读法：**

1. ceiling 从 100% 崩到 1.4%（低于随机线 7.1%）——**原集的满分全部来自词表泄漏**，这是铁证。
2. SUT 35.8% 是 ceiling 的 25 倍——去掉关键词提示后规则彻底失效，模型仍能做对三分之一。
3. 35.8% 的绝对值不高，如实报告。

> 三臂设计的关键是 ceiling 臂：它回答"不用 AI，最笨的规则能做到多少"。跑不赢 ceiling 的 AI 系统是纯粹的浪费。

### 10.3 评分口径演进

最初报 87.1%，那是**子串匹配**（期望工具名出现在全文任意位置即算命中）。两个问题：`survival_analysis` 是 `tmb_survival_analysis` 的子串，永远误判；"提到即命中"等于送分。

改为与 ceiling 同口径的**严格 top-1**（只解析 `recommendations[0].tool.pipeline_id`）后降至 35.8%。三口径并列：

| 口径 | 数值（67 例） | 含义 |
|---|---|---|
| 子串（已弃用） | 85.1% | 提到即算 |
| **严格 top-1** | **35.8%** | 与 ceiling 可比 |
| 格式合规率 | 52% | 半数写散文 |
| 结构化子集内准确率 | 69% | 遵守契约时的选择质量 |

### 10.4 两个系统性发现

**生存族 0/5。** `km_survival`/`cox_model`/`survival_analysis`/`tmb_survival_analysis`/`her2_pfs_survival` 意图高度重叠，模型稳定选到"合理但非标答"的那个。这是**题库单解 vs 图谱多解**的标注问题，不是模型缺陷。

该发现推动手册 §12.1 增加同族判别：`survival_analysis` 按**指定基因突变状态**分组、`tmb_survival_analysis` 按 **TMB 中位数**分组、按**基因表达水平**分组的是 `her2_pfs_survival`。

**输出契约仅执行 52%。** 即使系统提示词强制"输出单个 JSON"，模型仍近半写散文。

**该发现直接决定架构：模型格式合规率只有一半，校验层必须存在且必须在服务端。** 不能指望提示词写得够严。前端接入因此要求三道断言：① `schema_version: "tool-chain/v2"`；② `validate_plan` 返回 `grounded=true`；③ 提交前 `submittable=true`。失败回灌重试 2–3 次，仍失败如实报错。

### 10.5 数据面

96 例期望文件图内可查 **174/186（93.5%）**，精确命中 **138/186（74.2%）**，宽匹配 36（19.4%）。6 例（q052–057）期望的是 demo 文件（`NVM0598_*`、`ENCSR142YZV_chr19only_*`），不在图内——以图为准，如实报 `missing_from_graph`。
...
- 不受模型端点影响的三项确定性测试全部重跑并通过（见 §10.7），改动的正确性有独立证据。
...
| 降级行为 | 模型不可用时退回词表（去名集 1.4%） | 无降级，要么模型在环要么如实报错 | 不以 1.4% 冒充正常输出 |
...
| 回归手段 | 96 例含水集 | 三臂 + 去名集 + 24 例门禁 + 模板审计 | 评估方法被重建 |
| 评估诚实度 | 96 例 100%（泄题） | 严格 top-1 35.8%，并列旧口径 | 数字更难看但可信 |
...
> 205MB 重 MCP → 110KB。96 例上，一张 60 行关键词表即可与重服务打平——**该任务的工具选择本身是平凡的，重架构从未在此挣到收益**。真正需要模型智能的是"去名"的意图理解。

这段话承认了打平的那 96 例本来就不难。真正的价值是：用 1/1800 的体积拿到同样结果；建立了能区分好坏的评测；在真考题上规则崩到 1.4% 而模型仍有 35.8%。
...
**扩大去名集。** 现 70 例覆盖 14/51 个工具，不到三成。全覆盖才能看清哪些工具族真的分不开。
...
### 12.3 远期：从图谱内 51 个工具到全量生信工具

**现状边界**：闭集 51 个工具（11 原子 + 38 pipeline + 1 task_pipeline），都在图里、都有格式契约和 `next_tool` 边。可靠性绝对，但天花板低——用户问 ATAC-seq、Seurat 单细胞聚类、nf-core RNA-seq，只能回 `unsupported`。

**生态规模参照**：Bioconda 9,000+ 包、nf-core 100+ 流程、Galaxy ToolShed 8,000+ 工具、Bioconductor 2,300+ R 包、bio.tools 约 30,000 条注册记录。当前覆盖 51 个。
...
**方案对比。** 同一需求给 2–3 个方案并标注数据要求、预期产出、计算量。现为严格 top-1（为与 ceiling 可比而有意为之），但对真实用户未必最优。
...
| **去名集** | 去掉全部规则触发词、只写意图的测试集，用于暴露关键词表的虚假高分 |
| **三臂评测** | floor（随机）/ ceiling（纯规则）/ SUT（被测系统） |
| **严格 top-1** | 只看第一推荐是否命中，与 ceiling 同口径 |
