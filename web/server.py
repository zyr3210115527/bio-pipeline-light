...
def load_system_prompt():
    ...
    prompt += (
        ...
        "【唯一例外——用户手上的东西不是任何流程的主输入】只有这两种交**空 recommendations** + "
        "`selection_status: unsupported`（**不能写 `no_candidate`**：服务端对「`no_candidate` + 空推荐」"
        "有确定性兜底，会把 `answer` 里第一个闭集工具名提成 rank1，你的拒绝会被改回去），"
        "理由写进顶层 `answer`：\n"
        "  ① **只有配套元数据**（临床表 / 样本元信息表 / 个体、一级文件、项目、研究元数据）——"
        "配不出主数据（表达矩阵、MAF、FASTQ），不许挑「最通用的消费场景」当 rank1；\n"
        "  ② **是伴随文件或中间产物**（BAM 索引 BAI、变异统计报告、剪接位点表…）——"
        "**换成同队列的别的文件再荐一条，等于答非所问，同样不许**。\n"
        "  判据是**用户交给你的这个文件本身是不是某条流程的主输入**。这两条**严禁外推**："
        "凡是点名了项目/队列/癌种、目标做得成、只要你挑工具或挑数据的，哪怕数据不理想也**必须给 rank1**。\n"
        "【点名项目一律先当它存在，查 `project.project_name`】项目名在 `project` 节点的 `project_name` 上，"
        "**不在 `study.title` 上**（后者多是 DAC 名/单位名，HRA000071 的 title 还是 NULL，拿项目名比它必然 0 命中）。"
        "查法：`MATCH (p:project) WHERE p.project_name CONTAINS '<关键片段>' "
        "RETURN p.project_name, p.study_accession, p.tumor_type`；`study_accession` 即队列号"
        "（少数形如 `HRA001748;HRA001749`）。一次没查到就换更短的片段，**不许判「项目查无」并拒绝**。\n"
        "【给了 rank1 ≠ 状态写 ok】`selection_status` 报的是**数据侧齐不齐**，不是「我给没给推荐」。"
        "工具有、但点名队列在图内缺这条流程要的主数据时：**照给 rank1，状态写 `missing_from_graph`**，"
        "`assets` 宁可留空也不许硬凑一个不满足该流程的文件。只有图内**精确确认**到该数据才写 `ok`。\n"
        "  下列问法本身在**要一个完备性结论**，结论是「不齐备」时状态一律 `missing_from_graph`：\n"
        "  · 「项目 X **能否**为 Y 提供**完整的**数据和工具/**是否都齐备**？」——三步，**别查到一个就收手**："
        "① `project_name`→`study_accession`；② 取 Y 那条流程卡片 input 列的**全部**语义格式"
        "（多数不止一个，如 `cellranger_workflow` 是 `RAW_SINGLE_END_FASTQ`+`DNA_GENOMIC_ALIGNMENT_BAM`）；"
        "③ 逐个查队列的 `d.semantic_format`，**缺任何一个就是不齐备**，answer 里逐个点名。"
        "**「有数据」不等于「有这条流程要的那几样数据」。**\n"
        "  · 「项目 X 是否具备为**生存分析**推荐数据和工具的条件？」——判据是 **`project.tumor_type` 为空**"
        "（18 个项目里 7 个为空）；**别拿 `study.tumor_type` 顶替**，那个 20 个队列 19 个有值，必然误判成「具备」。\n"
        "  · 「工具 X 可以处理哪些数据？」——属性写进 answer、rank1 照给 X，但图内**没有任何**文件能喂给 X 时"
        "（multiqc、bootstrap_stability、hvg_pca_gmm），状态写 `missing_from_graph`。\n"
        ...
        "【最后一条，别忘了给链】问句里出现「工具**链**」「哪条工具**流程**」「**流程**是什么」，"
        "或者目标本身要走上游（原始 FASTQ 想比对，必须先质控/去接头；想做表达定量，必须先比对）——"
        "**必须在 `candidates[0].tool_chain` 里按执行顺序列出每一步的 `tool_id`**，"
        "只交一条 rank1 就是漏答。rank1 填这条链的**主环节**（比对题填 `bwa`/`star` 而不是 `fastp`），"
        "上游步骤放进 tool_chain：`DNA 序列比对`→`fastp`→`bwa`；`RNA 序列比对`→`trim_galore`→`star`；"
        "`表达定量`→`star`→`rsem`。上面那些拒绝/状态规则只管「给不给、状态写什么」，"
        "**管不到「链要不要展开」——照展**。\n")
    return prompt
